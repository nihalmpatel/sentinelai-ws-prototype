...
import {HumanReview, IrreversibleAction, ReviewType} from "../models/human-review.model";
...
export class CaseManager {
	...
	createCase(userId: number, context?: Record<string, unknown>): Case {
		const nextId = mockCases.length > 0 ? Math.max(...mockCases.map((c) => c.id)) + 1 : 1;
	...
	submitHumanReview(params: {
		...
		if (reviewType === "APPROVE_AI") {
			...
			if (score > LOW_RISK_APPROVAL_THRESHOLD) {
				...
				throw new Error("Approve AI recommendation is only available for low-risk cases. Use override instead.");
			}
		}

		const finalAction = reviewType === "APPROVE_AI" ? latestAi.recommendedAction : (params.finalAction as RecommendedAction);

		const rationale =
			reviewType === "APPROVE_AI" ? params.rationale?.trim() || "Approved AI recommendation." : (params.rationale as string).trim();
		...
		const targetStatus = reviewType === "APPROVE_AI" ? "APPROVED" : ("OVERRIDDEN" as const);
	...
	/**
	 * Find historical cases that are similar to the given case (FR-FE-03).
	 * Similarity is based on overlapping risk-signal IDs and comparable risk scores.
	 * Returns up to `limit` cases, excluding the case itself.
	 */
	findSimilarCases(caseId: number, limit = 5): Case[] {
		const target = this.getCase(caseId);
		if (!target || !target.riskProfile) return [];

		const targetSignalIds = new Set(target.riskProfile.signals.map((s) => s.id));
		const targetScore = target.riskProfile.score;

		type Scored = {c: Case; similarity: number};
		const scored: Scored[] = [];

		for (const c of mockCases) {
			if (c.id === caseId) continue;
			if (!c.riskProfile) continue;

			// Signal overlap
			const otherSignalIds = c.riskProfile.signals.map((s) => s.id);
			const overlap = otherSignalIds.filter((id) => targetSignalIds.has(id)).length;
			const union = new Set([...targetSignalIds, ...otherSignalIds]).size;
			const jaccard = union > 0 ? overlap / union : 0;

			// Score proximity (inverse of distance)
			const scoreDist = Math.abs(targetScore - c.riskProfile.score);
			const scoreProximity = 1 - scoreDist; // 0..1

			const similarity = jaccard * 0.6 + scoreProximity * 0.4;
			if (similarity > 0.1) {
